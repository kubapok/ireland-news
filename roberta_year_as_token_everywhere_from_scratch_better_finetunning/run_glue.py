...
def main():
    ...
    def preprocess_function(examples):
        ...
        for column in 'date', 'day_of_month', 'day_of_year', 'month', 'year', 'weekday', 'year_cont':
            if column == 'year':
                #t[column] = [[examples[column][i] - 1995] * t.input_ids[i].index(2) + [0] *(len(b) - b.index(2)) for i in range(len(t.input_ids))]
                t[column] = [[a-1995] * b.index(2) + [0] *(len(b) - b.index(2)) for a,b in zip(examples[column], t['input_ids'])]
            else:
